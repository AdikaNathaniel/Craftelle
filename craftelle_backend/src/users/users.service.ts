...
export class UsersService {
  ...
  async create(createUserDto: CreateUserDto) {
    try {
      ...
      const existingEmail = await this.userDB.findOne({ email: createUserDto.email });
      if (existingEmail) {
        throw new Error('An account with this email already exists.');
      }

      if (createUserDto.username) {
        const existingUsername = await this.userDB.findOne({ username: createUserDto.username });
        if (existingUsername) {
